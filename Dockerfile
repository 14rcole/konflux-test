...
RUN ARCH=$(uname -m) && echo https://github.com/open-policy-agent/conftest/releases/download/v"${conftest_version}"/conftest_"${conftest_version}"_Linux_"$ARCH".tar.gz
RUN echo https://mirror.openshift.com/pub/openshift-v4/"$ARCH"/clients/ocp/stable/openshift-client-linux.tar.gz
RUN echo "https://github.com/bats-core/bats-core/archive/refs/tags/v$BATS_VERSION.tar.gz"
RUN echo https://github.com/operator-framework/operator-registry/releases/download/"${OPM_VERSION}"/linux-amd64-opm
RUN echo https://github.com/opencontainers/umoci/releases/download/"${UMOCI_VERSION}"/umoci.amd64
